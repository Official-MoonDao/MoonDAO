...
describe('PR-0 DePrize geo gate', () => {
  ...
  it('keeps a region notice next to predictions and deletes the full-page notice', () => {
    ...
    expect(
      fs.existsSync(path.join(UI_ROOT, 'components/deprize/detail/RegionBanner.tsx'))
    ).to.equal(false)
  })
})
