...
var analyticsPage_default = {
  ...
  optOut: createDataTestSelector('onboarding-import-with-srp-button'),
}
...
var recoveryStep = {
  ...
  secretRecoveryPhraseWord: (index) =>
    createDataTestSelector(`import-srp__srp-word-${index}`),
  secretRecoveryPhraseWords: (index) =>
    createDataTestSelector(`srp-input-import__srp-note`),
...
var HomePage_default = {
  ...
  copyAccountAddressButton: createDataTestSelector('app-header-copy-button'),
...
var fixCriticalError = async (page, maxRetries = 5) => {
  for (let times = 0; times < maxRetries; times++) {
    if ((await page.locator(ErrorSelectors.criticalError).count()) > 0) {
      console.log(
        `[fixCriticalError] Metamask crashed with critical error, refreshing.. (attempt ${
          times + 1
        })`
      )
      if (times <= 3) {
      ...
      console.log(
        `[fixCriticalError] Metamask crashed with error, refreshing.. (attempt ${
          times + 1
        })`
      )
      if (times <= 4) {
...
async function confirmSecretRecoveryPhrase(page, seedPhrase) {
  ...
  await page.locator(StepSelectors.secretRecoveryPhraseWords(0)).click()
  await page.locator(StepSelectors.secretRecoveryPhraseWords(0)).fill('test')
  await page.keyboard.press('Space')
  //await page.locator(StepSelectors.secretRecoveryPhraseWords(0)).fill(' ')
  ...
  //await page.evaluate(() => navigator.clipboard.writeText(seedPhrase))
...
async function importWallet(page, seedPhrase, password) {
  ...
  await page.locator(createDataTestSelector('metametrics-no-thanks')).click()
  await page.locator(createDataTestSelector('onboarding-complete-done')).click()
  await page.locator(createDataTestSelector('download-app-continue')).click()
  //await page.locator(createDataTestSelector('pin-extension-done')).click()
  //confirm-footer-button
  //await page.locator(createDataTestSelector('tour-cta-button')).click()
...
async function confirmConnection(notificationPage, extensionId) {
  console.log('connecting')
  console.log('notificationPage')
  //console.log(notificationPage)
  console.log(await notificationPage.locator('confirm-btn'))
  const locator = notificationPage.locator('confirm-btn')
  console.log('content')
  console.log(await notificationPage.content())
  //await locator.waitFor({ state: 'visible' }) // Wait for the element to be visible
  //locator.click({ force: true })
  notificationPage.locator('button:text("Connect")').click()
  await notificationPage
    .locator('button:text("Confirm")')
    .waitFor({ state: 'visible' }) // Wait for the element to be visible
  notificationPage.locator('button:text("Confirm")').click()

  console.log('connected')
  //const notificationPageUrl = `chrome-extension://${extensionId}/notification.html`
  //const isNotificationPage = (page) => page.url().includes(notificationPageUrl)

  //console.log('the context.pages')
  //console.log(notificationPage.context().pages())
  //const theNotificationPage = notificationPage
  //.context()
  //.pages()
  //.find(isNotificationPage)
  //await theNotificationPage.locator('confirm-btn').click()
  //await theNotificationPage.locator('confirm-btn').click()
  //await notificationPage.locator('confirm-footer-button').click()
  //await notificationPage.locator('confirm-footer-button').click()
}
async function connectToDapp(notificationPage, accounts, extensionId) {
  if (accounts && accounts.length > 0) {
    await connectMultipleAccounts(notificationPage, accounts)
  }
  await confirmConnection(notificationPage, extensionId)
}
...
var NotificationPage = class {
  ...
  async connectToDapp(extensionId, accounts) {
    ...
    await connectToDapp(notificationPage, accounts, extensionId)
  }
...
var metaMaskFixtures = (walletSetup, slowMo = 0) => {
  return test.extend({
    ...
    context: async ({ context: currentContext, _contextPath }, use) => {
      ...
      const hash = '532f685e346606c2a803'
      ...
      await _metamaskPage
        .locator(createDataTestSelector('onboarding-complete-done'))
        .click()
      await _metamaskPage
        .locator(createDataTestSelector('download-app-continue'))
        .click()
      await _metamaskPage
        .locator(createDataTestSelector('pin-extension-done'))
        .click()
      await _metamaskPage
        .locator(createDataTestSelector('tour-cta-button'))
        .click()
      await _metamaskPage.close()
      await use(context)
      await context.close()
    },
