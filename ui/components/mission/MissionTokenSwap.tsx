...
import { V4_ROUTER_ADDRESSES, FEE_HOOK_ADDRESSES } from 'const/config'
...
export default function MissionTokenSwap({
  tokenAddress,
}: {
  tokenAddress: string
}) {
  ...
  async function swap() {
    ...
    const input = ethers.utils.defaultAbiCoder.encode(
      ['bytes', 'bytes[]'],
      [actions, [exactInput, params1, params2]]
    )
  ...
  return (
    <div className="mt-4 space-y-2">
      ...
      />
      <StandardButton onClick={swap}>Buy Tokens</StandardButton>
    </div>
  )
}
