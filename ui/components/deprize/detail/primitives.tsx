...
export function StateBadge({
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-xs font-medium border ${tone}`}
      title={title}
    >
...
export type NoticeItem = NoticeItemBase<ReactNode>

export { pickNotices } from '@/lib/deprize/noticeStack'

export function NoticeStack({ items }: { items: NoticeItem[] }) {
  const visible = pickNotices(items)
  if (visible.length === 0) return null
  return (
    <>
      {visible.map((item) => (
        <Notice key={item.id} tone={item.tone}>
          {item.body}
        </Notice>
      ))}
    </>
