...
export function ProjectRewards({
  ...
  // The retroactive pool is the *remainder* after upfront project budgets
  // are paid (and after the 10% Community Circle carve-out), NOT the full
  // quarterly pool. For each cycle we hardcode the post-upfront remainder
  // in `const/config.ts`:
  //   - ETH-paid cycles → `RETRO_ETH_BUDGET`
  //   - USDC-paid cycles → `RETRO_USD_BUDGET`
  // The on-chain CSV `getPayouts` generates uses `native,,address,amount`
  // rows for ETH (which means ETH on Arbitrum) and standard token rows
  // for USDC. The asset symbol drives both the budget magnitude pulled
  // here and the unit shown on the page.
  const isEthPayoutCycle = RETRO_PAYOUT_TOKEN === 'ETH'
  const retroPrimaryBudget = isEthPayoutCycle ? RETRO_ETH_BUDGET : RETRO_USD_BUDGET
    ...
    <section id="projects-container" className="overflow-hidden">
      ...
      <Container>
        <ContentLayout
          ...
          isProfile
        >
          <div className="mt-2 flex flex-col gap-3 sm:gap-6">
