...
export default function ActionLabel({ action }: { action: Action }) {
  ...
  if (action.type === 'Request Budget') {
    ...
    requestBudget.multisigTeam.forEach((member) => {
      const key = member.discordUserId || member.address;
      if (memberMap[key]) {
        memberMap[key].isSigner = true;
      } else {
        memberMap[key] = {
          ...member,
          isRocketeer: false,
    ...
    const combinedTeam = Object.values(memberMap);
              ...
              {combinedTeam.map((member) => (
                <div
                  ...
                  className={classNames(
                    'ml-2',
                    (member.isRocketeer || member.isSigner) && 'font-semibold'
                  )}
                >
                  ...
                  <div className="text-xs opacity-80">
                    <AddressLink address={member.payoutAddress || member.address} />
                  </div>
                </div>
              ))}
            </div>
