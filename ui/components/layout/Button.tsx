...
export default function Button({
  ...
  const hasCustomBorderRadius = className.includes('rounded')

  const baseStyles = `
    font-medium transition-all duration-200 shadow-lg hover:shadow-xl
    ${hasCustomBackground ? '' : getVariantStyles()}
    ${hasCustomPadding ? '' : getSizeStyles()}
    ${hasCustomBorderRadius ? '' : borderRadius || 'rounded-lg'}
    ${hasCustomTextColor ? '' : textColor || ''}
    ${className}
