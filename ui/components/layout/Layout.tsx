...
export default function Layout({ children, lightMode, setLightMode }: any) {
  ...
  const layout = (
    <main className={`${!lightMode && 'dark'} relative z-10`}>
      ...
      <div className="drawer drawer-mobile stars-bg">
        ...
        <div className="drawer-side bg-black">
          <label htmlFor="side-drawer" className="drawer-overlay z-10"></label>
          <div className="sidebar w-80 flex flex-col justify-between pb-24 lg:pb-0 overflow-y-auto drop-shadow-md min-h-screen">
            ...
            <ul className="menu scrollbar-hide p-4 overflow-y-auto text-base-400 grow font-GoodTimes text-[90%]">
              {nav.map((item: any, i: number) => (
                <NavigationLink item={item} key={'li' + i} />
              ))}
            </ul>
          </div>
        </div>
      ...
      {/*User BLOCKY, Connect buttons, added PINK and HIDDEN for debugging, this is to eb included in the sidebar itself, INSIDE*/}
      <ul className="bg-pink-500 hidden menu p-4 text-base-400">
