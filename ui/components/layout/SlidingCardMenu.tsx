...
const SlidingCardMenu: React.FC<SlidingCardMenuProps> = ({ children }) => {
  ...
  return (
<div ref={parentRef} className="relative rounded-tl-[20px] rounded-bl-[5vmax] p-4">
  <div 
    ref={containerRef}
    className="md:px-20 flex overflow-x-auto overflow-y-hidden"
    style={{ msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }}
  >
    {children}
  </div>
  <div id="left-fadeout"
    className="hidden md:block left-fadeout h-full w-[80px] bg-red-500 absolute top-0 left-0 z-50"
  ></div>
  <div id="right-fadein"
