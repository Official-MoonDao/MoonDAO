...
export default function SafeTransactions({ address, safeData }: SafeTransactionsProps) {
  const {
    safe,
  ...
  return (
    <div className="max-h-[600px] overflow-y-auto" data-testid="transactions-section">
      <h3 data-testid="transactions-title" className="text-lg font-GoodTimes mb-3 text-white">
        Recent Transactions
      </h3>

      {pendingTransactions.length > 0 && (
        <div data-testid="transactions-list" className="space-y-4">
          {groupTransactionsByNonce(pendingTransactions).map(({ nonce, transactions }) => {
            const hasRejectionInGroup = transactions.some(
              (tx) =>
                ...
                ((tx.data === '0x' || tx.data === null) && ethers.BigNumber.from(tx.value).eq(0))
            )
              ...
              >
                {transactions.length > 1 && (
                  ...
                  >
                    <p className="text-yellow-500 text-sm">
                      ⚠️ Multiple transactions with the same nonce ({nonce}). Only one can be
                      executed.
                    </p>
                  </div>
                )}
                ...
                {transactions.map((tx: any) => {
                  ...
                  const hasSigned = confirmations.some(
                    (conf: any) => conf.owner.toLowerCase() === address?.toLowerCase()
                  )
                  const canExecute =
                  ...
                  const isEthTransfer =
                    (tx.data === '0x' || tx.data === null) && ethers.BigNumber.from(tx.value).gt(0)
                  ...
                  return (
                    ...
                    >
                      ...
                      <p
                        ...
                        className="text-gray-300 mb-2"
                      >
                        Confirmations: {confirmations.length}/{tx.confirmationsRequired || 0}
                      </p>
                      ...
                      <SafeTransactionData
                        ...
                        onToggle={() =>
                          setExpandedTx(expandedTx === tx.safeTxHash ? null : tx.safeTxHash)
                        }
                      />
                      ...
                      >
                          ...
                          <>
                            {canSign && (
                              <PrivyWeb3Button
                                ...
                                className="rounded-full px-4 py-2 text-sm font-medium min-h-[36px]"
                                label="Sign"
                            ...
                            <PrivyWeb3Button
                              ...
                              className="rounded-full px-4 py-2 text-sm font-medium min-h-[36px] bg-slate-700/60 hover:bg-slate-700/80 border border-slate-600/40 text-slate-300 hover:text-white"
                              label="Reject"
                              action={() => handleRejectTransaction(tx.safeTxHash)}
                              noPadding
                              noGradient
                            />
                          </>
                        ...
                        {canExecute && (
                          <PrivyWeb3Button
                            ...
                            className="rounded-full px-4 py-2 text-sm font-medium min-h-[36px] bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 border border-blue-400/30"
                            label="Execute"
                        ...
                        {hasSigned && !tx.isExecuted && !hasRejectionInGroup && (
                          <PrivyWeb3Button
      ...
      {isNetworkMismatch ? (
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
          <p className="text-yellow-500 mb-3">
            Please switch to {selectedChain.name} to view transactions.
          </p>
          <Button
            variant="gradient"
            size="sm"
