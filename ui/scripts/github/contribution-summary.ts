...
WIP: The AI summary is generated using OpenAI API, every 20 commits are processed in a batch.  The final summary is generated by combining the summaries of all batches.
...
async function searchCommits(
  ...
  console.log(
    `🔍 Searching for merged commits by ${username} from ${startDate} to ${endDate}...`
  )
  ...
  try {
    ...
    console.log(`✅ Found ${allCommits.length} commits total`)
    return allCommits
  } catch (error) {
...
async function getContributionSummary(
  ...
  console.log(`\n📊 Getting contribution summary for ${username}`)
...
function displayStandardFormat(result: SearchResult, username: string): void {
  console.log('\n' + '='.repeat(80))
  console.log(`📈 CONTRIBUTION SUMMARY FOR ${username.toUpperCase()}`)
  console.log('='.repeat(80))

  console.log(`\n💾 COMMITS (${result.commits.length} total):`)
  ...
  if (result.commits.length === 0) {
    ...
    result.commits.forEach((commit, index) => {
      ...
      if (commit.commit.message.includes('\n')) {
        ...
        console.log(
          `   📝 Full message: ${fullMessage.replace(/\n/g, '\n       ')}`
        )
      }
    })
  }
...
async function callOpenAIForBatch(
  ...
  console.log(
    `🤖 Generating summary for batch ${batchNumber}/${totalBatches}...`
  )
...
Please provide a concise but comprehensive summary of this batch. This will be combined with other batch summaries later.`
  ...
  try {
    ...
    console.error(`❌ Error calling OpenAI for batch ${batchNumber}:`, error)
    throw error
  }
}
...
This summary should be professional, comprehensive, and suitable for performance reviews or project reports. Synthesize insights from all batches to provide a complete picture of the work accomplished.`
  ...
  try {
    ...
    console.error('❌ Error calling OpenAI for final summary:', error)
    throw error
  }
}
...
async function generateAndSaveSummary(
  ...
  try {
    ...
    console.log(`📊 Processing ${result.commits.length} commits...`)

    const batches = createBatches(result, 50) // 20 items per batch
    ...
    for (let i = 0; i < batches.length; i += concurrencyLimit) {
      ...
      const chunkPromises = chunk.map(async (batch, chunkIndex) => {
        ...
        try {
          ...
          console.error(
            `❌ Failed to process batch ${batchIndex + 1}, skipping...`
          )
          return {
            index: batchIndex,
