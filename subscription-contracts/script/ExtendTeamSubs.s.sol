...
contract ExtendTeamSubsScript is Script, Config {
    address TEAM_CONTRACT = readAddress("MoonDAOTeam");
    address DISCOUNT_LIST_CONTRACT = readAddress("TeamDiscountList");
    ...
    function getAllValidTeamTokenIds(MoonDAOTeam teamContract) internal view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= maxTokens; i++) {
            // Skip blocked teams
            if (isTokenBlocked(i)) {
                console.log("Skipping blocked team token");
                continue;
            }
