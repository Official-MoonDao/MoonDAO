...
- `subscription-contracts/test/deprize/DePrizeMint.t.sol` (new — 28 unit + 3 guarded fork tests)
...
- **`setMarket(deprizeId, market)`** (onlyOwner) — binds a market and **validates the wiring**: `market.pmSystem() == ctf`, `market.collateralToken() == weth`, `market.atomicOutcomeSlotCount() == teamIds.length`, and `market.conditionIds(0) == registry.getDePrize(deprizeId).ctfConditionId`. This makes a misconfigured or wrong-condition market un-bindable rather than failing at bet time.
...
`forge test --match-path 'test/deprize/DePrizeMint.t.sol'` — **31 passing**:

**`DePrizeMintTest` (28, deterministic, no RPC)** — real `DePrizeRegistry` + lightweight mocks for the JB terminal, WETH, CTF, and an LMSR (linear-price) market that faithfully models the fee and the self-calling `tradeWithTWAP`:
...
- edge cases: `NonPositiveCost` (qty 0), residual-collateral sweep + `receive()`, single `onERC1155Received` delivery, empty-buffer forward (no tokens minted), `RefundFailed` (refund recipient reverts), exact-budget bet with no refund;
- upgradeability: `_authorizeUpgrade` owner success + non-owner revert;
- ERC-1155 guard: unsolicited single/batch transfers and non-CTF sender revert; `supportsInterface` (ERC1155Receiver + ERC165, unknown id false).

Coverage of `DePrizeMint.sol` is `100%` functions / `100%` branches / `96%` lines (the 3 unhit lines are init/constructor calls that execute but are misattributed under via-ir coverage). A root `codecov.yml` excludes `*.t.sol` / `*.s.sol` / `prediction/` so coverage measures the contracts, not the tests/scripts.
