...
> (placeholder only). The G6 purse is a **community payload purchase** with the
> waterfall in [DEPRIZE_TOUCHDOWN.md](DEPRIZE_TOUCHDOWN.md) Part VI and
> [DEPRIZE_PAYLOAD_PURSE.md](DEPRIZE_PAYLOAD_PURSE.md), not a cheque to a CLPS operator.
> Terms v1.2 govern. This GTM's phase plan is unchanged.
...
| **G4** | **No single key controls the money** | **The top technical gate.** v1 on Arbitrum: `0x3c5e…E011` was simultaneously the CTF oracle, both UUPS proxy owners and (via `DePrizeFeeRouter`) the market owner, with untimelocked upgrades. **v2 removes the mechanism, not just the key:** `DePrizeRegistry` and `DePrizeMint` are immutable (`Ownable2Step`, no `initialize` / `upgradeToAndCall`, `renounceOwnership` disabled), there is no fee router, and the admin Safe owns every stock Gnosis market directly. What remains is the Safe's own signer policy. The v2 registry/mint/market must be deployed fresh with `DEPRIZE_OWNER` = the Safe; `DePrizeVerify.s.sol` asserts every owner and the oracle are that Safe before open | Any of oracle, registry owner, mint owner or market owner is not the Safe, or the v1 proxies are still in the value path |
...
| **G7** | **Exposure is bounded, by an audit or by a cap** | The pricing and custody math is **unmodified Gnosis CTF + LMSR**. In v1 a focused audit ([`DEPRIZE_SECURITY_AUDIT.md`](./DEPRIZE_SECURITY_AUDIT.md)) found one High in our `tradeWithTWAP` subclass — proof the residual risk was our glue. **v2 deletes that glue class:** no market subclass, no fee router, no proxies; the mint fails closed on any collateral or token drift. Exposure is now bounded by evidence rather than by size: `DePrizeMintInvariant` (mint holds nothing, slices reach Juicebox, collateral accounted) and `StockLmsrFork` (real Gnosis bytecode, ETH conserved to the wei) run in CI, `SurfaceAllowlist` pins the ABI, Slither fails on medium. The generation-1 per-bet cap (`DEPRIZE_MAX_BET_WEI`) is a product choice, not a substitute for these | Money is taken through a market not created by the stock factory, or through the v1 mint, or with the invariant / fork suites red |
