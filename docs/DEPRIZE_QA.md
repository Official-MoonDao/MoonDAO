...
| **Open Field Team NFT** (roster-changes) | **Minted.** Sepolia token id **24** on the current `TEAM_ADDRESSES.sepolia` MoonDAOTeam contract. `OPEN_FIELD_TEAM_ID = 24` in `ui/lib/deprize/competitions.ts`. Minted while seeding DePrize 10 (crewed-lander) via `DePrizeSeedRace.s.sol` and reused as the last outcome on every tech-tree race seeded since — see `docs/DEPRIZE_TECH_TREE_RACES.md`. DePrize 9's live roster does **not** include a field slot (predates this convention). Note: `ownerOf(301..303)` now reverts on this contract, so DePrize 9's teams are not resolvable here — display for DePrize 9 is unaffected because the UI always supplies `nameOverride` for bound races, but this is worth a closer look separately (possibly a redeployed `MoonDAOTeam` instance since DePrize 9 was seeded). |
