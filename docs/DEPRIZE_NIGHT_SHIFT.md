...
That is the prize. Everything below is the precise version of that sentence. There is **one
milestone**. You either cleared it or you didn't.
...
| | v0.5 | v0.6 |
|---|---|---|
| Opening | History first; the one-sentence bar on page 3 | **The sentence is the first thing on the page** |
| 98% rule | Easy to read as a 9.8 W waiver | **Slope limit on top of a 10.0 W floor** — finishing at 9.8 W fails |
| Watts on the Moon | A source line in the appendix | **Contrast box in Part I** — different challenge, say so |
| Attendance | "Two attended days" misread as 48 h of standing watch | **Day 0 + Day 15** on the one-pager; 350 h unattended |
| Mid-run evidence | Seals + video, underspecified | Live public telemetry, clock-in-frame video, hash-chained logs, optional remote check-in |
| Verification load-bearing path | Route B (DNV / TÜV, ~$3–8k) | **Route A default** (accredited lab report). Route C funded for Open Field. Route B only if someone can actually get that quote |
| Thermal wadi / size | Mass on the scoreboard only | **Mass and occupied volume are published** so a warehouse-sized heat store cannot hide |
| 10 We | Unchanged | **Unchanged** — raising watts without a size cap just rewards a bigger chamber |

**Changes from v0.3**, all in response to earlier advisor review:
...
This document supersedes `DEPRIZE_SURVIVE_THE_NIGHT_SPEC.md`. The one-pager companion is
[`night-shift-brief.html`](night-shift-brief.html) (v0.6). Print that to PDF if you need a file;
any v0.5 `Night_Shift_Prize_Brief.pdf` is stale.

### How this is not NASA Watts on the Moon

NASA's Centennial [Watts on the Moon Challenge](https://www.nasa.gov/prizes-challenges-and-crowdsourcing/centennial-challenges/watts-on-the-moon-challenge/) asked teams to **store and distribute** night power for a simulated lunar outpost. Finals ran in Glenn's vacuum chambers. The work was power management, wiring, and a multi-node energy network.

Night Shift is a different question: **one unplugged article, 10 We, 354 hours, buy-the-fuel-on-an-invoice.** No distribution network. No wall power standing in for a heat source. No NASA-selected finalists. We keep the industrialization framing and we say the two challenges are not 1:1.
...
| **The bar** | The sentence at the top of this document. ≥ 10 We every rolling hour for **354 hours**, ≤ 100 K, ≤ 1×10⁻⁵ torr, nothing plugged in, last hour still ≥ 10 We and ≥ 98% of the first hour |
| **Milestones** | **One** |
...
| **Size disclosure** | Article **mass** and **occupied chamber volume** are published. Not a cap — a scoreboard so a thermal wadi cannot hide in a warehouse |
| **Where** | Any thermal-vacuum chamber, anywhere in the world |
| **Deadline** | None — rolling sunset, open until solved |
| **Roster** | 7 named systems + Open Field (thermal-wadi / latent-heat stores compete here unless they claim a named slot) |
...
The sentence is at the top of this document. Repeated here so Part II stands alone:

> **Deliver at least 10 watts of electricity, continuously, for 354 hours, in a vacuum chamber
> colder than 100 kelvin, with nothing plugged in — and still be delivering the same 10 watts at
> the end. Publish the data.**

"The same 10 watts" means **still ≥ 10.0 We in the last hour**, not 9.8 We. The 98% rule is the
slope on top of that floor. Everything below is the precise version of that sentence. There is
**one milestone**. You either cleared it or you didn't.
...
**Why 10 We, and why we are not raising it.** Four times Chang'e-4's entire nighttime electrical
budget. It is the line between *a thermometer running while the spacecraft sleeps* and *a lander
doing work* — a real radio, a real computer, a heated instrument. The test is terrestrial, which
...
**This is not a 2% waiver on ten watts.** The rolling-hour floor in §5 is independent and
absolute: `min(rolling_60min_mean(P)) >= 10.0`. A flat 10.0 W run passes both. A run that starts
at 10.0 W and sags to 9.8 W fails the floor even though it would pass 98%. A run that starts at
20 W and finishes at 10.1 W clears the floor and **fails 98%** — that is a consumable burning
down, which is the problem we are trying to retire.

Zero margin on the slope fights thermocouple noise and calibration drift. 98% is a little slope
limit so measurement error is not a disqualification, and so a battery cannot start fat and
finish at the floor. Keep it.
...
| Route | Who | Cost to entrant | Role |
|---|---|---|---|
| **A** | An ISO/IEC 17025-accredited test laboratory hosting the run | **~$0 marginal** — this is already what a test report is | **Default.** Funded entrants go here |
| **C** | A MoonDAO sealed metrology package plus a MoonDAO-appointed observer for Day 0 and Day 15 | Shipping and observer travel, **paid from the purse** for Open Field | Open Field and anyone who cannot book Route A |
| **B** | An independent verification body attending the entrant's own chamber — DNV, TÜV, Bureau Veritas, Lloyd's Register, Element, NTS, Intertek | **Whatever they actually quote.** The ~$3–8k figure in v0.5 is a hope, not a bid | Optional, and only if a written quote exists before T0 |
...
Route A is the load-bearing path. An accredited lab already owns the meters and already signs test
reports; that is the product. Route B is not exotic in principle — those firms certify batteries and
hydrogen systems — but a fifteen-day cryogenic vacuum claim for a few thousand dollars has not been
bid. Do not design as if it has. Route C exists so a university team is not priced out while we wait
for a quote.

Video and a pretty CSV are **not** a substitute for any of these routes. The cheat is a hidden
conductor, a wall-powered heater, or the entrant's own meter. The root of trust is independent
custody of the instruments. Video is corroboration.

### 9.3 The observation protocol — Day 0 and Day 15, not 48 hours of standing watch

This is not a person sitting the chamber for two days, or for fifteen. It is **two site visits**.

**Day 0.** The verifier weighs and photographs the article, records occupied volume, applies
tamper-evident seals, inventories every conductor at the feedthrough panel and records what each
one carries, installs and calibrates **their own** load and environment sensors, starts the hash
chain, starts the public telemetry, and starts the witness video with a clock in frame.
...
- **Live public telemetry** of load V/I and shroud T / pressure, at the same rates as §8.2
- **Continuous or near-continuous video** of the article, the feedthrough panel, and a **clock
  in frame** (a visible NTP-set timepiece is enough). Gaps from signal drops are allowed; they
  must appear as gaps in the hash chain, not as spliced video
- **Hash-chained logs** so a missing minute is obvious and a looped recording fails
- **Optional remote human check-in** about every 72 hours — a person steps into frame, or the
  verifier pings the live stream. Not a third site visit
...
**Thermal wadis, latent-heat stores, and high-specific-heat piles compete here.** There is not yet
a single commercial product to name as Slot 8 — the published line of work is still research
(Hayne / ASU / PSI and related regolith-thermal-mass concepts; Australian and Japanese
night-survival thermal storage). They are the first-class non-nuclear threat: a large thermal mass
in a large chamber can look like 10 We for 354 hours. That is why mass and occupied volume are
published, and why raising the watt bar without a size rule would make them *more* competitive, not
less.
...
| JAXA | No named night-power *product*; LUPEX rover dropped RHUs. Night-survival research and ISAS TVAC stay Open Field |
| CSA / KARI / Australian TVAC groups | Facility and research, not a named article. Listed as chamber hosts in Part VI |
...
**occupied volume**, source and price. Most cells will start empty or say *"not publicly demonstrated."*
...
(Wh delivered per kg), **specific energy by volume** (Wh per occupied m³), **output retention**,
**cost per watt-year**, and **projected repeat cycles**. Occupied volume is the convex envelope of
the article as mounted, including any dedicated thermal mass that counts as part of the article.
Chamber walls and host-owned fixtures do not count. There is no cap — publish the number. A
warehouse-sized heat store will show up here even if it clears 10 We.

This is where a 40 kg battery, a 4 kg RTG, and a thermal wadi that all cleared the same bar get
told apart in public without either being disqualified.
...
| ESA ESTEC — Large Space Simulator / Phenix | European teams. Neutral host. Strongest non-US option on paper |
| DLR Bremen / Lampoldshausen | German teams. Neutral host |
| CSA David Florida Laboratory | Canadian teams. Neutral host. CSA itself is not a named competitor |
| JAXA / ISAS Sagamihara | Japanese teams. Neutral host if JAXA is not a competitor. If JAXA enters Open Field, they cannot also host |
| KARI | Korean teams. Same conflict rule |
| ISRO / IIT | Indian teams. Same conflict rule |
| ANSTO / Australian university TVAC | Australian teams. Same conflict rule |

Glenn remains a conflict of interest for US government competitors. A Glenn chamber is usable for a
non-NASA US team (Venturi, Perpetual Atomics) or an Open Field team, not for NASA Glenn's own
article.
...
qualifying evidence, never the only one. Listing a chamber here is a starting point, not a booking.

---

# PART VII — OPEN QUESTIONS

v0.6 closed four arguments that v0.5 left open. They are recorded here so the next person does not
reopen them by accident.

**Closed: the 10-watt floor.** Stays at 10 We. A higher floor, in isolation, crowns the RTG and
...
## Appendix B — Validator schema and adversarial fixtures

The validator is a program, not a vibe. It reads one CSV per run and returns `pass` or `fail` plus
the failing row. Teams may publish a second, richer log (CAN bus, spectrometer, camera hashes). The
official log is this schema. If the official log is incomplete, the run is incomplete.

### B.1 Official CSV schema

One row per sample. Sample interval ≤ 1 second. Header required, column order required, units as
stated.
...
`hash_self = sha256(row_without_hashes + hash_prev)`.
...
A 60-second clip of a passing first minute, concatenated 21,240 times. Voltage, current, and power
are perfect. `t_unix` jumps backward or stalls at the loop boundary, or the hash chain (if present)
repeats.

Expected validator output: `FAIL monotonic_time at row 61` (or `FAIL hash_chain at row 61`).

A loop that also forges monotonic timestamps still fails the optional ~72-hour check-in: the
inspector's live reading will not match the log at that unix time.

### B.4 Failing fixture — 9.8 W waiver

A team that read "≥ 98%" as "9.8 W is fine." First hour mean 10.00 W. Last hour mean 9.82 W.
Ratio 0.982 — the slope test would pass if it were the only test. Rolling-hour mean drops below
10.0 W at hour 200.

Expected validator output: `FAIL rolling_hour_floor=9.82 at t_unix=...` (the first hour-window whose
mean is < 10.0 W).
...
1. Satisfies B.1.
2. Gets `PASS` from the rules in §8.
3. Describes an article that is not a night-shift power plant.

If you can do that, the tests are wrong. The next draft closes the hole. A verbal description of a
cheat is useful; a CSV that actually passes is the thing that changes the brief.
