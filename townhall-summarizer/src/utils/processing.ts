...
export async function getTranscript(
  videoId: string,
  groq: Groq,
  whisperModel: string = DEFAULT_MODELS.whisper
): Promise<string> {
  const source = (process.env.TRANSCRIPT_SOURCE || "auto").toLowerCase();

  if (source === "audio") {
    console.log("TRANSCRIPT_SOURCE=audio — skipping captions, going straight to yt-dlp + Whisper.");
  ...
  try {
    ...
    if (captions) {
      console.log(
        `Using caption-based transcript (${
          captions.isAutoGenerated ? "auto-generated" : "manual"
        }, ${captions.transcript.length} chars). Skipping audio download.`
      );
  ...
  if (source === "captions") {
    ...
    throw new Error(
      `No caption track available for video ${videoId} and TRANSCRIPT_SOURCE=captions. ` +
        ...
        `or set TRANSCRIPT_SOURCE=auto to fall back to audio transcription.`
    );
  }
...
export async function formatSummaryForConvertKit(
  summary: string,
  videoTitle: string,
  videoDate: string,
  videoId?: string
